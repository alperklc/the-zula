...
import { CaretCoordinates, getCaretCoordinates } from './util/TextAreaCaretPosition'
import { insertText } from './util/InsertTextAtPosition'
import { mod } from './util/Math'
import { SuggestionsDropdown } from './suggestions-dropdown'
import { ButtonHTMLAttributes, DetailedHTMLFactory, TextareaHTMLAttributes } from 'react'
...
export class TextArea extends React.Component<TextAreaProps, TextAreaState> {
  ...
  getTextArea = (): HTMLTextAreaElement => {
    return this.props.refObject?.current as HTMLTextAreaElement
  }
  ...
  startLoadingSuggestions = (text: string) => {
    ...
    this.currentLoadSuggestionsPromise = this.currentLoadSuggestionsPromise!.then(() =>
      loadSuggestions?.(text, this.state.mention.triggeredBy!),
    ).then((suggestions) => {
  ...
  handleKeyDown = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    switch (mention.status) {
      ...
      case 'active':
        if (
          key === 'Escape' ||
          (key === 'Backspace' && selectionStart <= (this.state.mention.startPosition as number))
        ) {
          // resetting suggestionsPromiseIndex will cause any promise that is yet to be resolved to have no effect
          // when they finish loading.
          this.suggestionsPromiseIndex = 0
          ...
          this.setState({
            mention: {
              ...mention,
              focusIndex: mod(
                (mention.focusIndex as number) + focusDelta,
          ...
          event.preventDefault()
          this.handleSuggestionSelected(mention.focusIndex as number)
        }
        break
      default:
      // Ignore
    ...
    switch (mention.status) {
      ...
      case 'active':
        if (key === 'Backspace') {
          const searchText = value.substr(
            mention.startPosition as number,
            this.getTextArea().selectionStart - (mention.startPosition as number),
          )
      ...
      case 'inactive':
        if (key === 'Backspace') {
          ...
          const isAtMention = (suggestionTriggerCharacters as string[]).includes(
    ...
    switch (mention.status) {
      ...
      case 'inactive':
        if (
          ...
          !/\s|\(|\[|^.{0}$/.test(value.charAt(this.getTextArea().selectionStart - 1))
        ) {
          return
        }
  ...
  render() {
    ...
    const suggestionsEnabled =
      suggestionTriggerCharacters && suggestionTriggerCharacters.length && loadSuggestions

    const { mention } = this.state

    const TextAreaComponent = (textAreaComponent || 'textarea') as DetailedHTMLFactory<
      TextareaHTMLAttributes<HTMLTextAreaElement>,
      HTMLTextAreaElement
    >
    ...
    return (
      <div style={{ position: 'relative' }}>
        <TextAreaComponent
          className={className}
          style={{ height: heightVal, resize: 'none' }}
