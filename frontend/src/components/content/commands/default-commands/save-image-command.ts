...
function dataTransferToArray(items: DataTransferItemList): Array<File> {
  ...
  for (const index in items) {
    ...
    if (item.kind === 'file') {
      result.push(item.getAsFile() as File)
    }
  }
  return result
}
...
function filterItems(
  ...
  if (accept) {
    ...
    const mimeTypes = new Set(acceptedTypes.filter((t) => /^[-\w.]+\/[-\w.]+$/.test(t)))
    const anyTypes = new Set(
      acceptedTypes.filter((t) => /(audio|video|image)\/\*/.test(t)).map((t) => t.split('/')[0]),
    )
...
export const saveImageCommand: Command = {
  async execute({ textApi, context }: ExecuteOptions): Promise<void> {
    ...
    for (const index in filteredItems) {
      ...
      const placeHolder = `${breaksBefore}!['uploadingImage']()`
...
function isPasteEvent(context: CommandContext): context is PasteCommandContext {
  return (
    ((context as PasteCommandContext).event as React.ClipboardEvent).clipboardData !== undefined
  )
}
